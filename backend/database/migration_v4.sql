-- ============================================================
--  MIGRATION V4 — Phone Verification, OTP & Session Security
--  MySQL compatible (works with MySQL 5.7+ and Aiven)
--
--  This migration ADDS columns/tables for:
--    - Phone number verification (OTP flow)
--    - JWT token blacklist (logout invalidation)
--    - OTP attempt tracking (rate limiting)
--
--  It does NOT drop or modify existing data.
--  All new columns have defaults so existing rows (including admin)
--  are unaffected. Existing users are auto-marked phone_verified=1.
--
--  Run:  mysql -u root -p < migration_v4.sql
...
-- Clean up old OTP attempt records (older than 24 hours)
