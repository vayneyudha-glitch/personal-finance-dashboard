/* ============================================================
   ADMIN-BUDGETS.JS — Budget Management
   ============================================================ */

requireAdmin();
renderSidebar('admin-budgets');
initSidebarMobile();
initThemeToggle();

var budgetState = { users: [], categories: [] };

async function loadBudgets() {
    var container = document.getElementById('budgetsTable');
    if (container) container.innerHTML = loadingHtml('Loading budgets...');

    var result = await AdminAPI.getBudgets();
    if (!result.ok) { container.innerHTML = errorStateHtml('Failed to load budgets'); return; }
    ...
    var html = '<div style="overflow-x:auto;"><table class="data-table"><thead><tr>' +
        '<th>ID</th><th>User</th><th>Category</th><th>Budget</th><th>Actual</th><th>Remaining</th><th>Progress</th><th>Status</th><th>Period</th><th>Actions</th>' +
        '</tr></thead><tbody>';
    items.forEach(function(b) {
        ...
        html += '<tr>' +
            ...
            '<td>' + b.period + '</td>' +
            '<td><div style="display:flex;gap:4px;"><button class="btn btn-sm btn-primary" onclick="editBudget(' + b.id + ', ' + b.amount + ', \'' + b.period + '\', \'' + b.startDate + '\', \'' + (b.endDate||'') + '\', ' + (b.categoryId||'null') + ', ' + b.userId + ')">Edit</button>' +
...
async function loadUsersAndCategories() {
    ...
    if (usersResult.ok) budgetState.users = usersResult.data.data.items;
    if (catsResult.ok) budgetState.categories = catsResult.data.data;
}
...
function showBudgetModal(isEdit, id, data) {
    ...
    var userOptions = budgetState.users.map(function(u) { return '<option value="' + u.id + '"' + (data && data.userId===u.id ? ' selected' : '') + '>' + escapeHtml(u.name) + ' (' + escapeHtml(u.email) + ')</option>'; }).join('');
    var catOptions = '<option value="">All Expense Categories</option>' + budgetState.categories.filter(function(c) { return c.type === 'Expense'; }).map(function(c) { return '<option value="' + c.id + '"' + (data && data.categoryId===c.id ? ' selected' : '') + '>' + escapeHtml(c.name) + '</option>'; }).join('');
    ...
    var html = '<div class="modal-backdrop" id="' + modalId + '">' +
        '<div class="modal">' +
            ...
            '<div class="modal-body">' +
                '<div class="form-group"><label>User</label><select class="form-input" id="budgetUser">' + userOptions + '</select></div>' +
...
function editBudget(id, amount, period, startDate, endDate, categoryId, userId) {
    showBudgetModal(true, id, { amount: amount, period: period, startDate: startDate, endDate: endDate, categoryId: categoryId, userId: userId });
}

async function submitBudget(id, modalId) {
    var data = {
        userId: parseInt(document.getElementById('budgetUser').value),
    ...
    var result = id ? await AdminAPI.updateBudget(id, data) : await AdminAPI.createBudget(data);
    if (result.ok) {
        closeModal(modalId);
...
function deleteBudget(id) {
    showConfirm('Delete this budget?', async function() {
        var result = await AdminAPI.deleteBudget(id);
